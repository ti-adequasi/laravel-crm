...
* [feature] PBX Inovalen integration, fourth step: a new "Histórico de Ligações" tab on the Lead-view screen, next to Descrição/Produtos/Cotações, listing every call the PBX has on record for that Person's phone numbers — including ones made or received directly on the desk phone, outside the CRM entirely — with a "Tocar" button to play back any call that has a recording, right on the page. This is a live, read-only view of the PBX's own call log, not a copy stored in the CRM, so it's always current and never needs syncing.
