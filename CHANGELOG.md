...
* [feature] First step of multi-tenancy: a new super-admin-only Tenants screen (Configurações-level, bottom of the sidebar) to create and manage tenants, and a nullable `tenant_id` column added to every tenant-owned table across the system (leads, contacts, products, quotes, emails, users, and the rest — ~40 tables in all). This step is purely additive — every column is nullable and nothing reads or filters by it yet, so existing behavior is completely unchanged; every current user is automatically a super-admin (no tenant assigned) until tenants and their users start actually being created. Real per-tenant data isolation is a separate, later step.
