...
* [feature] A new "Leads PeeringDB" prospecting screen (Configurações menu, alongside the existing "Leads Google" one) searches PeeringDB — the public directory of network operators, data centers, and organizations behind the internet's own infrastructure — by network/ISP, organization, or data center, with country/city/state/ASN/network-type filters fitted to whichever of those three is picked (a network has no country or city of its own in PeeringDB, so those fields are hidden rather than silently ignored when searching one), plus network scope and peering-policy filters and a data-center region filter for a finer search. Works like the existing Google-based flow: preview results first, pick which ones are worth keeping, import them, and optionally convert straight into an opportunity in a chosen pipeline. No API key is required at all — PeeringDB allows anonymous read access — an optional key only unlocks a few additional contact details PeeringDB otherwise hides from anonymous requests. A data center's own listed sales/technical contacts (when PeeringDB has them) come through directly, without needing to enrich a website first.

* [feature] Every imported PeeringDB prospect gets the same automatic website enrichment already built for Google-sourced prospects — email, WhatsApp, Instagram, Facebook, LinkedIn, and CNPJ company lookup — plus CSV and HTML export. A new progress banner on the prospect list shows how many imported prospects are still being enriched in the background and updates itself the moment the backlog clears, so a freshly-imported batch no longer looks like nothing is happening.

* [enhancement] Removed LGPD privacy-policy/DPO detection from both prospecting modules (Leads Google and Leads PeeringDB) — not a meaningful prospecting signal for either data source. Existing prospects keep everything else already gathered; nothing else about enrichment changes.
...
* [feature] PBX Inovalen integration, second step: each user can now be given a phone extension ("Ramal") on the Usuários settings screen, shown on the list and editable by the user themselves as well as an administrator.
...
* [feature] PBX Inovalen integration, fourth step: the Lead-view screen's existing "Chamadas" tab now also shows every call the PBX has on record for that Person's phone numbers — including ones made or received directly on the desk phone, outside the CRM entirely — alongside the calls already logged as Activities, sorted together by date, with a "Tocar" button to play back any recorded one right on the page. The PBX side of this is a live, read-only view of the PBX's own call log, not a copy stored in the CRM, so it's always current and never needs syncing.

* [feature] PBX Inovalen integration, fifth step: any recorded call in the Chamadas tab now also has a "Baixar" button (downloads the recording) and a "Transcrição e Resumo" button, showing the AI-generated summary, sentiment, and category alongside the full speaker-by-speaker transcript for a call already analyzed on the PBX side — or, for one that hasn't been yet, a "Gerar transcrição e resumo" button to request it.

* [feature] A Person's phone number is now checked at save time, on the Pessoa screen and on a Lead's own inline "new person" fields alike — it must already be entered with the leading "0" and area code (e.g. 011987654321 or 01132654321), the exact shape the PBX's own outbound rules require to complete a call, with a clear message if it isn't.

* [fixed] A real, non-administrator user (anyone belonging to a tenant) could save a Lead, Person, Product, or any other record with no field-level validation at all — required fields, e-mail format, phone format, custom regex rules, all silently skipped — because the underlying field-definition table was scoped to see only that exact tenant's own rows, and every one of those definitions has always been a shared, system-wide one belonging to no tenant in particular. Saves themselves weren't blocked, so this had no obvious symptom; only a super-admin session (which isn't scoped at all) was ever validating correctly. Every tenant now correctly sees the shared definitions again, on top of any of its own.
