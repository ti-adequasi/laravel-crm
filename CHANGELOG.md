...
* [feature] A new "Leads PeeringDB" prospecting screen (Configurações menu, alongside the existing "Leads Google" one) searches PeeringDB — the public directory of network operators, data centers, and organizations behind the internet's own infrastructure — by network/ISP, organization, or data center, with country/city/ASN/network-type filters fitted to whichever of those three is picked (a network has no country or city of its own in PeeringDB, so those fields are hidden rather than silently ignored when searching one). Works exactly like the existing Google-based flow: preview results first, pick which ones are worth keeping, import them, and optionally convert straight into an opportunity in a chosen pipeline. No API key is required at all — PeeringDB allows anonymous read access — an optional key only unlocks a few additional contact details PeeringDB otherwise hides from anonymous requests.

* [feature] Every imported PeeringDB prospect gets the same automatic website enrichment already built for Google-sourced prospects — email, WhatsApp, Instagram, Facebook, LinkedIn, CNPJ company lookup, and LGPD privacy-policy/DPO detection — plus CSV and HTML export. A new progress banner on the prospect list shows how many imported prospects are still being enriched in the background and updates itself the moment the backlog clears, so a freshly-imported batch no longer looks like nothing is happening.
