<?php

return [
    'menu' => [
        'title' => 'PBX',
        'info' => 'Conecte este tenant ao seu próprio domínio no PBX para discagem direta.',
    ],

    'acl' => [
        'title' => 'PBX',
        'calls-title' => 'Fazer Ligações',
    ],

    'settings' => [
        'title' => 'Configurações do PBX',
        'info' => 'Conecte este tenant ao seu próprio domínio no PBX Inovalen — a chave de API sozinha identifica a qual domínio uma requisição pertence, então todo tenant nesta instalação usa o mesmo servidor PBX, cada um com sua própria chave.',
        'enabled' => 'Habilitado',
        'auto-log-activity' => 'Registrar toda ligação automaticamente',
        'auto-log-activity-hint' => 'Quando ativado, toda ligação feita pelo CRM é adicionada à timeline do Negócio/Pessoa assim que ela termina — mesmo que a aba do navegador tenha sido fechada antes disso. Desative para fazer ligações sem adicionar itens na timeline; o histórico completo continua disponível de qualquer forma.',
        'api-key' => 'Chave de API',
        'api-key-hint' => 'Deixe em branco para manter a chave já salva.',
        'test-connection' => 'Testar Conexão',
        'testing' => 'Testando...',
        'test-success' => 'Conectado — esta chave pertence ao domínio ":domain".',
        'test-failed' => 'Falha na conexão: :error',
        'test-failed-generic' => 'Não foi possível testar a conexão. Tente novamente.',
        'test-missing-key' => 'Informe uma chave de API primeiro.',
        'save-btn' => 'Salvar',
        'saving' => 'Salvando...',
        'save-success' => 'Configurações do PBX salvas com sucesso.',
        'save-failed-generic' => 'Não foi possível salvar as configurações do PBX. Tente novamente.',
    ],

    'calls' => [
        'call-button' => 'Ligar',
        'hangup-button' => 'Encerrar',
        'calling' => 'Ligando...',
        'in-progress' => 'Ligação em andamento',
        'ended' => 'Ligação encerrada',
        'not-configured' => 'Este tenant ainda não tem uma conexão com o PBX configurada.',
        'no-extension' => 'Você ainda não tem um ramal configurado — configure um nas suas configurações de usuário primeiro.',
        'invalid-phone' => 'Isso não parece ser um número de telefone válido.',
        'originate-failed' => 'O PBX recusou a ligação: :error',
        'originate-failed-generic' => 'Não foi possível completar a ligação. Tente novamente.',
        'not-found' => 'Esta ligação não foi encontrada.',
        'not-yours' => 'Esta ligação pertence a outro usuário.',
        'hangup-failed-generic' => 'Não foi possível encerrar a ligação. Ela já pode ter terminado.',
        'activity-title' => 'Ligação para :phone',
        'activity-comment' => 'Ligação feita para :phone a partir do ramal :ramal.',
        'history-person-not-found' => 'Esta pessoa não foi encontrada.',
        'history-fetch-failed' => 'Não foi possível carregar o histórico de ligações do PBX: :error',
        'history-fetch-failed-generic' => 'Não foi possível carregar o histórico de ligações do PBX. Tente novamente.',
        'history-empty-title' => 'Nenhuma ligação ainda',
        'history-empty-description' => 'Ligações feitas ou recebidas nos números desta pessoa — pelo CRM ou direto no telefone de mesa — vão aparecer aqui.',
        'history-missed' => 'Perdida',
        'history-inbound' => 'Recebida',
        'history-outbound' => 'Feita',
        'history-local' => 'Interna',
        'history-play' => 'Tocar',
        'history-pause' => 'Pausar',
        'history-loading-recording' => 'Carregando...',
        'recording-fetch-failed' => 'Não foi possível carregar esta gravação: :error',
        'recording-fetch-failed-generic' => 'Não foi possível carregar esta gravação.',
        'history-download' => 'Baixar',
        'history-transcript-button' => 'Transcrição e Resumo',
        'history-summary-label' => 'Resumo',
        'history-transcript-label' => 'Transcrição',
        'history-sentiment-label' => 'Sentimento',
        'history-score-label' => 'Nota',
        'history-no-analysis-yet' => 'Esta ligação ainda não foi transcrita.',
        'history-analysis-pending' => 'Ainda processando — isso pode levar alguns instantes.',
        'history-generate-analysis' => 'Gerar transcrição e resumo',
        'history-generating' => 'Gerando...',
        'intel-fetch-failed' => 'Não foi possível carregar a transcrição/resumo: :error',
        'intel-fetch-failed-generic' => 'Não foi possível carregar a transcrição/resumo. Tente novamente.',
        'analyze-failed' => 'Não foi possível iniciar a análise: :error',
        'analyze-failed-generic' => 'Não foi possível iniciar a análise. Tente novamente.',
        'analyze-started' => 'Análise iniciada.',
    ],
];
