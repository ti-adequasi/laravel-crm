...
/**
 * Brazilian company-registry (CNPJ) lookup — identical to
 * Webkul\LeadGreen\Services\CnpjService. Ported rather than shared: this
 * class has zero coupling to either module's prospect model (it only ever
 * takes/returns plain strings and arrays), but each self-contained
 * prospecting package owns its own copy, matching this codebase's module
 * convention (see crm-package-development skill). If a third such module
 * ever needs it, that's the point to extract a shared package instead.
 */
class CnpjService
{
    /**
    ...
    /**
     * The CNPJá commercial API key — set from the LeadPeering settings
     * screen (Configuration > PeeringDB Leads), falling back to
     * `.env`/`config('services.cnpja.key')` for an ops-managed deploy that
     * prefers not to store secrets in the database. A separate value from
     * LeadGreen's own cnpja_api_key setting — each self-contained module
     * is configured independently, even though both call the same external
     * registries.
     */
    protected function cnpjaKey(): ?string
    {
        return core()->getConfigData('lead_peering.settings.api_keys.cnpja_api_key')
            ?: config('services.cnpja.key');
    }

    protected function cnpjaDailyLimit(): int
    {
        return (int) (core()->getConfigData('lead_peering.settings.api_keys.cnpja_daily_limit')
     ...
     *
     * Deliberately the exact same bare key LeadGreen's own CnpjService
     * uses — not scoped to this module or to the configured key. The real
     * daily limit lives on the CNPJá account itself, not on either CRM
     * module, and a business is overwhelmingly likely to have exactly one
     * such account: if the same real key ends up configured in both
     * LeadGreen's and LeadPeering's settings (the realistic case — there's
     * no reason to pay for two commercial subscriptions to serve two
     * internal features), matching LeadGreen's key format here means both
     * modules correctly draw against one shared counter and can't jointly
     * overspend the account's true limit. A module- or key-scoped cache key
     * would get that realistic case wrong (each module wrongly believing it
     * has the full quota to itself) in exchange for only correctly handling
     * the unlikely case of two genuinely different CNPJá accounts.
     */
