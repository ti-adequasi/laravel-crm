...
/**
 * Website enrichment (email, socials, WhatsApp, CNPJ, LGPD/DPO signals) —
 * identical to Webkul\LeadGreen\Services\LeadEnrichmentService. Ported
 * rather than shared: enrichFromWebsite() takes a plain website string and
 * returns a plain array with zero coupling to either module's prospect
 * model, but each self-contained prospecting package owns its own copy,
 * matching this codebase's module convention (see crm-package-development
 * skill). If a third such module ever needs it, that's the point to
 * extract a shared package instead.
 */
class LeadEnrichmentService
{
    ...
    protected function fetch(string $url): ?string
    {
        try {
            ...
            Log::info('LeadPeering enrichment fetch failed: '.$url.' - '.$e->getMessage());
        }
    ...
    protected function verifyEmail(string $email): ?bool
    {
        try {
            ...
            Log::info('LeadPeering enrichment Disify verification failed for '.$email.': '.$e->getMessage());
